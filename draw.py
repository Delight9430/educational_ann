...
class Chart:
    ...
    def set_key(self, cluster_1, cluster_2):
    ...
    def draw_prediction(self, epoch, input_points, predictions, loss):
        ...
        ax_inset.set_title("predicted", fontsize=9)

    def draw_key(self):
        if self.cluster_1 == None or self.cluster_2 == None:
            return
        ax_inset = self.ax.inset_axes([0.7, 0.01, 0.4, 0.4])

        # Plot inside the inset
        for _, point in enumerate(self.cluster_1):
            ax_inset.plot(point[0], point[1], marker="o", color=(0.0, 0.0, 1.0))
        for _, point in enumerate(self.cluster_2):
            ax_inset.plot(point[0], point[1], marker="o", color=(1.0, 0.0, 0.0))
        ax_inset.set_xlim(-8, 8)
        ax_inset.set_ylim(-8, 8)
        ax_inset.set_title("actual", fontsize=9)
